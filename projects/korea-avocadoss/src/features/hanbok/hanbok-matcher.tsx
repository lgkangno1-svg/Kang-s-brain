...
type HanbokLook={id:LookId;color:ColorId;moods:readonly MoodId[];comforts:readonly ComfortId[];destinations:readonly DestinationId[];seasons:readonly SeasonId[];jeogoriColor:string;chimaColor:string};

const HANBOK_LOOKS:readonly HanbokLook[]=[
 {id:'look1',color:'jadeIvory',moods:['elegant','romantic'],comforts:['balanced','photoFirst'],destinations:['hyangwonjeong','stoneWall'],seasons:['springAutumn','summer'],jeogoriColor:'#B8D0D8',chimaColor:'#4A1525'},
 {id:'look2',color:'roseNavy',moods:['minimal','romantic','royal'],comforts:['walking','balanced','photoFirst'],destinations:['stoneWall','bukchon'],seasons:['springAutumn','winter'],jeogoriColor:'#C8B6DB',chimaColor:'#1E293B'},
 {id:'look3',color:'moonBlue',moods:['royal','kdrama','elegant'],comforts:['photoFirst','balanced'],destinations:['bukchon','hyangwonjeong'],seasons:['winter','springAutumn'],jeogoriColor:'#FDFBF7',chimaColor:'#D4A373'}
];

function scoreLook(look:HanbokLook,choices:{color:ColorId;mood:MoodId;comfort:ComfortId;destination:DestinationId;season:SeasonId}){
 return (look.color===choices.color?40:0)+(look.moods.includes(choices.mood)?25:0)+(look.comforts.includes(choices.comfort)?15:0)+(look.destinations.includes(choices.destination)?10:0)+(look.seasons.includes(choices.season)?10:0);
}
...
export function HanbokMatcher(){
 ...
 return <div className="prototypePanel" id="hanbok-matcher">
  ...
  <div className="hanbokMatcherGrid">
   ...
   <fieldset className="hanbokFieldset"><legend>{t('seasonLegend')}</legend><div className="hanbokChoiceGrid">{SEASON_IDS.map(id=><label className={`hanbokChoice ${season===id?'hanbokChoiceActive':''}`} key={id}><input checked={season===id} name="hanbok-season" onChange={()=>setSeason(id)} type="radio" value={id}/><span>{t('seasons.'+id)}</span></label>)}</div></fieldset>
  </div>

  <div className="hanbokResultCard">
   <div className="hanbokResultHeader"><span className="hanbokResultEyebrow">{t('resultEyebrow')}</span><h3>{t('directions.'+mood+'.name')}</h3><p>{t('directions.'+mood+'.reason')}</p></div>
   <div className="hanbokResultSummary"><div><small>{t('resultPalette')}</small><strong>{t('colors.'+color)}</strong></div><div><small>{t('resultTripPriority')}</small><strong>{t('comfort.'+comfort)}</strong></div></div>
