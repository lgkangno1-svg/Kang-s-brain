...
export function HanbokRentalFinder(){
 ...
 const rows=useMemo(()=>GYEONGBOKGUNG_HANBOK_RENTALS.map(shop=>({shop,availability:hanbokRentalAvailabilityAt(shop,date,time)})).filter(({shop,availability})=>(!hideClosed||availability!=='closed')&&(language==='any'||shop.interpretationLanguages.includes(language))).sort((a,b)=>({open:0,recheck:1,closed:2}[a.availability]-{open:0,recheck:1,closed:2}[b.availability])),[date,time,language,hideClosed]);
 ...
 return <section id="rental-finder" style={{maxWidth:1120,margin:'28px auto 72px',padding:'0 20px'}}>
  ...
  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:14}}>{rows.map(({shop,availability})=><article key={shop.id} style={{border:'1px solid rgba(0,0,0,.12)',borderRadius:12,padding:16,background:'#fff'}}>
