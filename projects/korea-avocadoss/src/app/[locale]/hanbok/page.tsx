...
export default async function HanbokPage({params,searchParams}:PageProps){
  const [{locale},query]=await Promise.all([params,searchParams]);setRequestLocale(locale);
  const copy=PREVIEW_COPY[(locale in PREVIEW_COPY?locale:'en') as Locale];
  const rentalShops=await getGyeongbokgungHanbokRentalShops();
  const initialDate=validDate(one(query.date));
  const initialTime=validTime(one(query.time));
  return(
