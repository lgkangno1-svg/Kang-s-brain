...
function finalizeIdempotency(
  ...
  return {
    ...state,
    idempotency: [...state.idempotency, {key, fingerprint, resultEntryId}],
  };
}
...
export function applyWalletCommand(state: WalletState, command: WalletCommand): WalletCommandResult {
  ...
  switch (command.type) {
    case "grant": {
      ...
      ({state: nextState, entry} = appendEntry(state, command, {
        kind: "grant",
        amount: command.amount,
        availableDelta: command.amount,
        reservedDelta: 0,
        spentDelta: 0,
        reference: `${command.source}:${command.reference}`,
      }));
      break;
    }
    ...
    case "reserve": {
      ...
      if (balance.available < command.amount) throw new Error("Insufficient available credits");
      ({state: nextState, entry} = appendEntry(state, command, {
        kind: "reserve",
        amount: command.amount,
        availableDelta: -command.amount,
        reservedDelta: command.amount,
        spentDelta: 0,
        reference: `${command.feature}:${command.usageId}`,
      }));
      nextState = {
        ...nextState,
        reservations: [...nextState.reservations, {
    ...
    case "capture": {
      ...
      ({state: nextState, entry} = appendEntry(state, command, {
        ...
        spentDelta: amount,
        reference: command.usageId,
        parentEntryId: reservation.reservationEntryId,
      }));
      const updatedReservation: CreditReservation = {
        ...reservation,
        capturedAmount: reservation.capturedAmount + amount,
        status: amount === remaining ? "closed" : "active",
      };
      nextState = updateReservation(nextState, updatedReservation);
      nextState = {
        ...nextState,
        captures: [...nextState.captures, {
          captureEntryId: entry.id,
          reservationEntryId: reservation.reservationEntryId,
    ...
    case "release": {
      ...
      ({state: nextState, entry} = appendEntry(state, command, {
        ...
        reservedDelta: -amount,
        spentDelta: 0,
        reference: command.reason,
        parentEntryId: reservation.reservationEntryId,
      }));
      const updatedReservation: CreditReservation = {
        ...reservation,
        releasedAmount: reservation.releasedAmount + amount,
        status: amount === remaining ? "closed" : "active",
      };
      nextState = updateReservation(nextState, updatedReservation);
      break;
    ...
    case "refund": {
      ...
      ({state: nextState, entry} = appendEntry(state, command, {
        ...
        spentDelta: -amount,
        reference: command.reason,
        parentEntryId: capture.captureEntryId,
      }));
...
export function assertWalletInvariants(state: WalletState): WalletBalance {
  ...
  for (const reservation of state.reservations) {
    const source = findEntry(state, reservation.reservationEntryId);
    if (source.kind !== "reserve") throw new Error("Reservation must point to a reserve ledger entry");
    if (reservation.capturedAmount + reservation.releasedAmount > reservation.reservedAmount) {
      throw new Error("Reservation accounting exceeds reserved credits");
    }
  ...
  for (const capture of state.captures) {
    const source = findEntry(state, capture.captureEntryId);
    if (source.kind !== "capture") throw new Error("Capture record must point to a capture ledger entry");
