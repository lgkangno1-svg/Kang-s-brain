...
| Food & cafe finder | HARDENING | Source-checked official tourism records, category filters, hours/address/dietary notes, checked-at/source link. Records older than 30 days (or carrying an invalid/future check date) fail closed to verify-only. Current source-published Gyeongbokgung Station proximity is now stored for six records and deterministically orders category/all results nearest-first; missing distance is not guessed and sorts after verified-distance records. Tailor Coffee's current official address was refreshed at the same time. Saved-place recovery, address copy and walking-map handoff remain zero-AI browser behavior. | Surface the source-backed distance/exit directly in localized result cards, expand verified inventory and source-refresh operations, and continue localization of business descriptions. |
...
3. Finish release verification for the 12-look My Korea Look catalog, authored P0 expansion presentation and public sample SEO surface, then continue edge-case/device/native-editorial QA without weakening the six-look `/hanbok` localization boundary.
4. Continue Explore source-refresh/geospatial hardening; Food Finder now orders by source-published station proximity without inventing missing distances, while direct localized proximity display, broader inventory refresh and walking calibration remain.
