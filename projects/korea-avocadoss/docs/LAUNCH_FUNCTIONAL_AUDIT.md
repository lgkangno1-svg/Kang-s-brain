...
| Hanbok matcher | HARDENING | Color/mood/comfort/destination/season ranking plus Personal Color bridge. Its authored six-look comparison subset has locale-native P0 presentation, source/license attribution and deterministic ranking. Rental-finder date/time handoff is sanitized in the Server Component so source-checked shop content remains present in server HTML instead of hiding behind a client `useSearchParams` bailout. Rental availability now fails closed to the existing six-locale `recheck` state when the source is more than 30 days old **at the selected visit date**, or when `checkedAt` is invalid; stale schedules also suppress the published-closing countdown instead of presenting stale hours as operational guidance. | Expand the separate Hanbok comparison subset only after native presentation is authored for added references; verified rental inventory entity layer, source-refresh operations and broader visual/device regression tests also remain. |
...
4. Continue Explore source-refresh/geospatial hardening; Hanbok rental schedules now fail closed by selected-visit-date freshness, while Gyeongbokgung holiday substitution, calibrated walking legs, map handoff, arrival-time/next-tour advisor and P0 hero-alt localization are covered by code-level contracts.
