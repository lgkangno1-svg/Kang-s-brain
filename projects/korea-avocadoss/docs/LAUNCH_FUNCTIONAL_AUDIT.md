...
| Personal Color | HARDENING | Browser-local JPEG/PNG/WebP validation, size/dimension/pixel caps, local Lab/skin-pixel analysis, lighting warnings, manual correction, Hanbok bridge, orientation-aware `createImageBitmap` decoding with a browser-local `<img>` fallback and explicit decoder resource cleanup. Validation now reads compressed JPEG/PNG/WebP dimensions before pixel decode, so over-36-MP photos are rejected before a full bitmap allocation and accepted photos are no longer decoded once merely for validation. Visitors can explicitly revoke the selected object URL, release the in-memory File reference, clear the derived result and recover to idle without persistence or network transfer. The scanner stays unmounted until the visitor explicitly accepts a six-locale browser-local disclosure. | Real-device/fixture coverage for rotated camera JPEGs and constrained-memory devices, plus final native/legal review of privacy/consent wording. |
...
2. Complete remaining Personal Color real-device fixtures and constrained-memory QA; header-first pixel-cap validation now removes an avoidable full-decode allocation, but representative rotated-camera/mobile evidence and final consent wording review remain.
