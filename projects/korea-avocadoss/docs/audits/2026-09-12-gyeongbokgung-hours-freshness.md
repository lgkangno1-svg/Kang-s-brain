...
Checked 2026-09-12 (Asia/Seoul local calendar date) against the Royal Palaces and Tombs Center primary pages already linked by the product:
...
`GYEONGBOKGUNG_VISIT_FACTS_CHECKED_AT` is refreshed to 2026-09-12 and the source contract now defines a 30-day maximum provenance age. `check-gyeongbokgung-visit-helper.mjs` derives the source age from the current Asia/Seoul calendar date and fails when the check date is in the future or older than 30 days. The source and palace are both in Korea, so this avoids a UTC date-boundary false failure without extending the freshness window. This makes a source re-check mandatory before any future green release can continue carrying stale palace hours/tour facts.
...
- a non-future `checkedAt` on the Asia/Seoul calendar,
