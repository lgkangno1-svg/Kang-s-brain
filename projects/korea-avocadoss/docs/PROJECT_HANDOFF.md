...
Production currently includes exact/approximate/unknown birth-time contracts; unknown time is valid reduced scope and never gets a guessed hour; exact/approximate clock input requires IANA timezone; true-solar mode additionally requires longitude; raw DOB/time/city/timezone/longitude/name/account identifiers are stripped from narrative payloads. Policies are explicit `midnight` / `jasi` / `splitJasi` and `civil` / `true-solar`.
...
## 5. Current review slice — true-solar clock correction
Branch `feat/korea-saju-true-solar-time` adds a deterministic, zero-network, zero-AI conversion layer based on NOAA/GML's published Equation of Time approximation:
...
- `npm run check:saju` updated to execute the new gate.
...
- it preserves `dayOffset` when the solar correction crosses a civil-date boundary;
- branch mapping is deterministic and tests prove both Singapore and Seoul hour-branch crossings;
...
This slice is not production-verified until exact branch-head MiniPC CI, merge, exact merged-SHA deploy, consecutive public checks and full P0 crawl all pass.
