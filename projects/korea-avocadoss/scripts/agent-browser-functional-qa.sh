...
expect_text(){ local needle="$1" label="$2" text;text="$(body)";if ! grep -Fq "$needle" <<<"$text";then diagnostics;fail "$label: missing text [$needle]";fi;pass "$label"; }
open_page(){ local path="$1";step "open ${path}";ab open "${ORIGIN}${path}" >/dev/null;ab wait 900 >/dev/null;ab snapshot -i >/dev/null; }
assert_no_horizontal_scroll(){ local label="$1" result;result="$(ab eval 'document.documentElement.scrollWidth <= window.innerWidth')";grep -Fq 'true' <<<"$result" || { diagnostics;fail "$label: horizontal overflow"; };pass "$label no-horizontal-scroll"; }
