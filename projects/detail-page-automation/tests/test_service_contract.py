...
class ContractTests(unittest.TestCase):
    ...
    def test_reference_selection_prefers_diversity_and_dedupes(self):
        files = [
            {"id": "a", "role": "product_full", "quality_score": .95, "product_identity_score": 1, "usefulness_score": .9, "duplicate_group_id": "g1"},
            {"id": "b", "role": "product_full", "quality_score": .9, "product_identity_score": .9, "usefulness_score": .8, "duplicate_group_id": "g1"},
            {"id": "c", "role": "cross_section", "quality_score": .8, "product_identity_score": .9, "usefulness_score": .95, "duplicate_group_id": "g2"},
            {"id": "d", "role": "packaging", "quality_score": .75, "product_identity_score": .8, "usefulness_score": .8, "duplicate_group_id": "g3"},
        ]
        picked = select_references(files, 3)
        self.assertEqual(len(picked), 3)
        self.assertEqual(len([x for x in picked if x.get("duplicate_group_id") == "g1"]), 1)
        self.assertTrue(any(x.get("role") == "cross_section" for x in picked))
