...
5. `scripts/generate_images.py`가 GPT Image API로 생성/편집, 정확한 규격으로 후처리
6. 상세 12장을 `detail_full.png`로 결합
7. `scripts/validate_run.py`가 구조/규격/필수 prompt chain/중복을 검증

## 핵심 설계 결정
- LLM 판단은 Codex가 담당하고, 템플릿 주입/파일 검증/이미지 호출은 deterministic Python으로 분리한다.
- 이미지 생성은 재실행 시 기존 파일을 건너뛰어 중간 실패에서 복구한다.
- 실제 제품 사진이 있으면 `images.edit` 경로를 사용해 제품 정체성을 최대한 유지한다.
...
- master 자동 주입 스크립트
- 이미지 생성/재시작/참조 이미지 입력
- 자동 규격 보정 및 detail_full 결합
- 검증 스크립트
- 사용법 README

## 다음 우선순위
1. 실제 상품 1개로 end-to-end 시험
2. GPT-Image-2의 한글 타이포 정확도 평가
3. 필요 시 “AI가 배경/제품 비주얼 생성 + HTML/Pillow가 한글 카피 오버레이” 하이브리드 렌더러 추가
4. 카테고리별 프롬프트 프리셋(신선식품/가공식품/생활용품 등)
5. 비용/품질 프리셋 및 선택적 재생성 UI

## 테스트
- `python scripts/build_master.py <q_json>`
- `python scripts/generate_images.py <jobs_json> --dry-run`
- `python scripts/validate_run.py <run_dir>`
- API 키가 있는 환경에서는 `--require-images`까지 실행
